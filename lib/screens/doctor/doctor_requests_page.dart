...
class _DoctorRequestsPageState extends State<DoctorRequestsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _bookings.isEmpty
              ? Center(child: Text('Sem agentamentos disponiveis'))
              : ListView.builder(
  ...
  void _showStatusDialog(String requestId, String currentStatus) {
    List<String> statuses = ['Aceito', 'Rejeitado', 'Completado'];
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              title: Text('Atualizar status da solicitação'),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('Por favor, selecione o status para esta solicitação.'),
